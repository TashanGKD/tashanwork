...
function getModelDisplayTitle(modelID: string, modelName?: string | null) {
  if (modelID.trim().toLowerCase().includes("big-pickle")) {
    return resolveModelDisplayName(modelID);
  }
  return modelName?.trim() || resolveModelDisplayName(modelID);
}
    ...
    const options = getConnectedProviderItems(data)
      .flatMap((provider) =>
        Object.entries(provider.models).map(([id, model]) => ({
          ...
          modelID: id,
          title: getModelDisplayTitle(id, model.name),
                        ...
                        >
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            <span className="block truncate text-xs text-muted-foreground">
                              {item.subtitle} - {denAuth.isSignedIn ? "订阅后添加此模型" : "登录后解锁"}
                            </span>
                          </span>
