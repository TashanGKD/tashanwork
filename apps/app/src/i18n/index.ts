...
export const t = (
  ...
  if (!params) return brandText(result);

  let out = brandText(result);
...
function brandText(input: string) {
  return input
    .replace(/OpenWork Cloud/g, "TaShanWork Cloud")
    .replace(/TashanWork Cloud/g, "TaShanWork Cloud")
    .replace(/OpenWork Models/g, "TashanWork 模型")
    .replace(/OpenWork Hub/g, "TashanWork Hub")
    .replace(/OpenWork/g, "TashanWork")
    .replace(/OpenCode Zen/g, "Tashan CLI")
    .replace(/OpenCode/g, "Tashan CLI")
    .replace(/opencode/gi, "tashan-cli")
    .replace(/\.tashan-cli/g, ".tashan");
}
...
export const initLocale = (): Language => {
  ...
  try {
    ...
    if (isLanguage(stored) && stored !== "en") {
      localeValue = stored;
      if (typeof document !== "undefined") {
