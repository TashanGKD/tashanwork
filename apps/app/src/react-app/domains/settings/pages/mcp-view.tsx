    ...
    <section className="space-y-8 max-w-3xl w-full animate-in fade-in duration-300">
      ...
      {props.builtInExtensionsDisabled ? (
        <div className="rounded-xl border border-amber-6 bg-amber-2 px-4 py-3 text-xs text-amber-11">
          内置工具能力已被组织策略禁用。可显示隐藏项查看被阻止的内置能力。
        </div>
      ) : null}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          >
            {showHidden ? "正在显示隐藏项" : hiddenOrPolicyCount > 0 ? `显示隐藏项 (${hiddenOrPolicyCount})` : "显示隐藏项"}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(min(100%,20rem),1fr))] gap-3">
        ...
        {(props.installedPlugins ?? []).map((plugin) => {
          ...
          return (
            <ExtensionCard
              ...
              name={plugin.name}
              description={plugin.description ?? `已安装 ${fileCount} 个文件的市场插件。`}
              kind="extension"
              connected={true}
              hidden={hidden}
              actionLabel="查看详情"
        ...
        {props.entries.length === 0 && (props.installedSkills ?? []).length === 0 && (props.installedPlugins ?? []).length === 0 ? (
          <div className="col-span-full rounded-xl border border-dashed border-dls-border px-5 py-10 text-center">
            ...
            <div className="text-sm font-medium text-dls-secondary">没有找到工具能力</div>
            <div className="mt-1 text-xs text-dls-secondary/60">换个关键词或筛选条件，或回到数字员工补充能力。</div>
          </div>
        ) : null}
      </div>
    </div>
