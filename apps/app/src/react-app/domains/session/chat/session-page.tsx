...
import { Bot, Columns2, Database, FileCheck2, FileText, Globe, Mic2, Settings2, Workflow, X, Zap } from "lucide-react";

import { t } from "../../../../i18n";
import { OPENWORK_EXTENSION_CATALOG } from "../../../../app/constants";
import { TASHAN_KNOWLEDGE_INDEX_PROMPT } from "../../../../app/tashan-workbench";
...
export function SessionPage(props: SessionPageProps) {
  ...
  const openBrowserUrlControlAction = useMemo<OpenworkControlAction>(() => ({
    ...
    description: "Create or select a TashanWork built-in browser tab, navigate it to a URL, and return the CDP handle for browser automation.",
    ...
    <div className="flex h-full min-h-0 flex-col bg-[radial-gradient(circle_at_top,rgba(74,111,255,0.12),transparent_42%),var(--app-bg,#0b1020)] text-dls-text mac:bg-transparent">
      ...
      >
        <AppSidebar
          ...
          onOpenSettings={props.sidebar.onOpenSettings}
          onOpenCommandPalette={props.sidebar.onOpenCommandPalette}
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <h1 className="truncate text-[15px] font-semibold text-dls-text">
                {showWorkspaceSetupEmptyState
                  ? "创建或连接项目空间"
                  : `当前会话：${selectedSessionTitle || t("session.default_title")}`}
              </h1>
              <span className="hidden truncate text-[13px] text-dls-secondary lg:inline">
                当前项目空间：{workspaceName} · 数字员工：按插件调用 · 当前模型：{providerCount > 0 ? "默认模型" : "未连接"}
              </span>
              {props.developerMode ? (
                ...
                <div className={`mx-auto max-w-[800px] px-6 ${showWorkspaceSetupEmptyState ? "pt-20" : "pt-10"}`}>
                  {props.notFoundMessage ? (
                    ...
                    <div className="space-y-6 px-6 text-center">
                      ...
                      <div className="space-y-2">
                        <h3 className="text-xl font-medium">创建或连接项目空间</h3>
                        <p className="mx-auto max-w-sm text-sm text-dls-secondary">
                          选择一个本地文件夹作为会话、资料、产物和权限审计的边界。
                        </p>
                      </div>
                      <div className="flex justify-center">
                        <Button onClick={props.sidebar.onOpenCreateWorkspace}>创建项目空间</Button>
                      </div>
                    </div>
                      ...
                      <div className="w-full max-w-md space-y-6">
                        <div className="space-y-1 text-center">
                          ...
                          <p className="text-xs text-dls-secondary">
                            {providerCount === 0
                              ? "接入可用模型后，数字员工才能执行项目空间内的企业工作流。"
                              : "每个员工会绑定当前项目空间，并创建会话、待办和权限确认。"}
                          </p>
                        </div>
                        <div className="space-y-2">
                          {providerCount === 0 ? (
                            ...
                            >
                              ...
                              <div>
                                <div className="text-[13px] font-medium text-dls-text">连接模型供应商</div>
                                <div className="mt-0.5 text-[11px] text-dls-secondary">
                                  使用企业模型、私有模型或 OpenAI 兼容 API 执行任务
                                </div>
                              </div>
                            </button>
                          ) : null}
                          <button
                            ...
                            onClick={() => {
                              props.sidebar.onCreateTaskWithPrompt?.(
                                ...
                                "作为他山企业数字员工，帮我设计一个客户资料整理工作流：读取工作区资料，识别客户字段，生成结构化表格方案，并列出需要人工确认的权限点。",
                              );
                            }}
                          >
                            <Bot className="mt-0.5 size-5 shrink-0 text-dls-secondary" />
                            <div>
                              <div className="text-[13px] font-medium text-dls-text">研究写作员工</div>
                              <div className="mt-0.5 text-[11px] text-dls-secondary">客户资料整理、方案写作和确认点设计</div>
                            </div>
                          </button>
                          <button
                            type="button"
                            ...
                            onClick={() => {
                              props.sidebar.onCreateTaskWithPrompt?.(
                                props.selectedWorkspaceId,
                                "为一个企业 AI 工作台演示创建任务流：包括资料输入、数字员工分工、工具调用、人机确认、执行计划、输出物和审计日志。",
                              );
                            }}
                          >
                            <Workflow className="mt-0.5 size-5 shrink-0 text-dls-secondary" />
                            <div>
                              <div className="text-[13px] font-medium text-dls-text">任务流编排员</div>
                              <div className="mt-0.5 text-[11px] text-dls-secondary">生成可执行、可追踪、可审计的流程</div>
                            </div>
                          </button>
                          <button
                            type="button"
                            ...
                            onClick={() => {
                              props.sidebar.onCreateTaskWithPrompt?.(
                                props.selectedWorkspaceId,
                                TASHAN_KNOWLEDGE_INDEX_PROMPT,
                              );
                            }}
                          >
                            <Database className="mt-0.5 size-5 shrink-0 text-dls-secondary" />
                            <div>
                              <div className="text-[13px] font-medium text-dls-text">企业知识管理</div>
                              <div className="mt-0.5 text-[11px] text-dls-secondary">生成知识索引、字段和检索方案</div>
                            </div>
                          </button>
                          <button
                            type="button"
                            ...
                            onClick={() => {
                              props.sidebar.onCreateTaskWithPrompt?.(
                                props.selectedWorkspaceId,
                                "检查当前企业工作台演示路径的权限和审计设计：文件访问、浏览器工具、命令执行、模型调用、人工确认和日志留痕分别应该如何呈现。",
                              );
                            }}
                          >
                            <FileCheck2 className="mt-0.5 size-5 shrink-0 text-dls-secondary" />
                            <div>
                              <div className="text-[13px] font-medium text-dls-text">权限审计员</div>
                              <div className="mt-0.5 text-[11px] text-dls-secondary">检查工具调用、人机确认和日志留痕</div>
                            </div>
                          </button>
                        </div>
                      </div>
