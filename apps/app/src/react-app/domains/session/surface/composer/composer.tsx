...
import { AppWindowMac, ArrowUp, Check, ChevronDown, ChevronRight, FileText, ListPlus, Paperclip, Plug, Settings, Sparkles, Square, Terminal, X, Zap } from "lucide-react";
...
export function ReactSessionComposer(props: ComposerProps) {
  ...
  const isAgentActive = useCallback(
    (name: string) => props.selectedAgent === name || (props.selectedAgent === null && name.toLowerCase() === BUILD_AGENT_NAME),
    [props.selectedAgent],
  );
  ...
  const handleKeyDownCapture: React.KeyboardEventHandler<HTMLDivElement> = (event) => {
    ...
    if (agentMenuOpen) {
      const total = nonDefaultAgents.length;
      if (total === 0) return;
      ...
      if (event.key === "Enter" || event.key === "Tab") {
        event.preventDefault();
        const selected = nonDefaultAgents[agentMenuIndex]?.name ?? nonDefaultAgents[0]?.name ?? null;
          ...
          >
            <div className="grid gap-1">
              {mentionFiltered.map((item, index) => (
                ...
                >
                  {item.kind === "employee" ? (
                    <Sparkles size={14} className="mt-0.5 shrink-0 text-violet-10" />
                  ) : item.kind === "agent" ? (
                        ...
                        <div className="max-h-72 overflow-y-auto p-2">
                          ...
                          {toolMenuSection === "extensions" ? (
                            composerExtensions.length > 0 ? (
                              ...
                              <div className="px-3 py-2 text-xs text-gray-10">暂无启用的内置扩展。可在设置中连接工具。</div>
                            )
                          ) : null}
                          {activePlugin ? (
                        ...
                        {nonDefaultAgents.map((agent, index) => {
                          ...
                          return (
                            <button
                              key={agent.name}
                              ref={(element) => {
                                agentItemRefs.current[index] = element;
                              }}
                              type="button"
                              className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-xs transition-colors ${active || agentMenuIndex === index ? "bg-gray-2 text-gray-12" : "text-gray-11 hover:bg-gray-2/70"}`}
