...
function SettingsRouteContent(props: SettingsSurfaceProps = {}) {
  ...
  const installOpenAiImageExtension = useCallback(async (apiKey: string) => {
    ...
    if (!openworkClient) {
      setImageExtensionError("TashanWork server is not connected.");
      return;
    }
    if (!resolvedApiKey) {
    ...
    try {
      ...
      setImageExtensionStatus("Saved OPENAI_API_KEY. Agents can use TashanWork extension actions for image generation.");
    } catch (error) {
  ...
  const generateOpenAiTestImage = useCallback(async (input: { apiKey: string; prompt: string }) => {
    ...
    if (!client || !workspaceId) {
      setImageGenerationError("TashanWork server is not connected for this workspace.");
      return;
    }
    if (!apiKey) {
  ...
  const testVoiceSession = useCallback(async () => {
    if (!openworkClient) {
      setVoiceError("TashanWork server is not connected.");
      return;
    }
    setVoiceBusy(true);
    setVoiceStatus(null);
    ...
    try {
      ...
      setVoiceStatus(`Realtime ready with ${session.model} (${session.tools.length} TashanWork tools).`);
    } catch (error) {
      setVoiceError(describeRouteError(error));
    } finally {
      setVoiceBusy(false);
  ...
  const installLocalProvider = useCallback(async (input: LocalProviderInstallInput) => {
    ...
    if (!client || !workspaceId) {
      setLocalProviderError("TashanWork server is not connected for this workspace.");
      return;
    }
    if (!modelId) {
  ...
  const handleSaveRenameWorkspace = useCallback(async () => {
    ...
    try {
      if (!openworkClient) {
        toast.error("TashanWork server is unavailable. Reconnect the server before renaming workspaces.");
        return;
      }
  ...
  const handleExportWorkspaceConfig = useCallback(async (workspaceId: string) => {
    ...
    throw new Error("TashanWork server is unavailable. Reconnect the server before exporting workspace config.");
  ...
  const handleCreateWorkspace = async (preset: WorkspacePreset, folder: string | null) => {
    ...
    try {
      ...
      if (!list) {
        throw new Error("TashanWork server is unavailable. Start or reconnect the server before creating a workspace.");
      }
  ...
  const handleCreateRemoteWorkspace = async (input: {
    ...
    try {
      ...
      if (!list) {
        throw new Error("TashanWork server is unavailable. Start or reconnect the server before connecting a remote workspace.");
      }
      const createdId = resolveWorkspaceListSelectedId(list) || list.workspaces[list.workspaces.length - 1]?.id || "";
      if (createdId) {
        await workspaceSetSelected(createdId).catch(() => undefined);
