...
function describeTaskCreateError(error: unknown) {
  ...
  if (
    ...
    return "Tashan CLI is unavailable for this workspace. Retry once it restarts, or restart TashanWork if the problem continues.";
  }
  return message;
}
...
export function SessionRoute() {
  ...
  const handleCreateTaskInWorkspace = useCallback(async (workspaceId: string) => {
    ...
    try {
      ...
      toast.error("Tashan CLI unavailable", {
        description: message,
        action: {
          label: "Retry",
  ...
  return (
    ...
    <SessionPage
      ...
      sidebar={{
        ...
        onCreateTaskWithPrompt: (workspaceId, prompt) => {
          void (async () => {
            ...
            try {
              ...
              setComposerDraft(session.id, prompt);
              writeActiveWorkspaceId(workspaceId || null);
              writeLastSessionFor(workspaceId, session.id);
              rememberPendingCreatedSession(workspaceId, session.id);
              setSessionsByWorkspaceId((current) => ({
        ...
        onOpenSettings: (route?: string) => handleOpenSettings(route ?? "/settings/general"),
